...
func setupScalingTarget(cfg config.Scaler, logF logging.LoggerFactory) (scaler.ScalingTarget, error) {
	...
	if cfg.Mode == config.ScalerModeNomadDataCenter {
		cfg := nomadWorker.Config{Logger: logF.NewNamedLogger("sokar.nomadWorker"), AWSRegion: cfg.Nomad.DataCenterAWS.Region, AWSProfile: cfg.Nomad.DataCenterAWS.Profile, ASGTagKey: "datacenter"}
